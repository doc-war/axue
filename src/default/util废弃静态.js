...
// import * as util from "../frame/util" 
// import { render,html} from 'lit';
// import { unsafeHTML } from 'lit/directives/unsafe-html.js';
// //把所有内置组件注册掉
// import "../elements/builtin/Tip"
// import "../elements/builtin/PropertyBar"
// import "../elements/builtin/Slot"
// import "../elements/builtin/ToastSlot"
// import "../elements/builtin/MenuY"     //其实循环引用了本util,但无碍，因为模块注册初始执行只有一次，只要不循环调用即可
...
// */
// export function appendChild(container, childHtml){
//     if(!container || !container instanceof Node){
//         console.warn("要追加的container类型非法：",container)
//         return 
//     }
//     //节点元素
...
//         let litNode
//         if(!slot){    
//             litNode= html`<axue-property-bar .args=${args}></axue-property-bar>`; 
//         }else if(slot instanceof Node || 1 === slot?.["_$litType$"]){    
//             litNode= html`<axue-property-bar .args=${args}><div slot="slot">${slot}</div></axue-property-bar>`; 
//         }else if( typeof slot === 'string'){ 
//              /**处理普通字符串，无论是不是模板 */
//             litNode= html`<axue-property-bar .args=${args}><div slot="slot">${unsafeHTML(slot)}</div></axue-property-bar>`; 
//         }else{ 
//             /**未知类型原封不动传入*/  
//             litNode= html`<axue-property-bar .args=${args}><div slot="slot">${slot}</div></axue-property-bar>`; 
//         }
...
//             return 
//         }
//         const moduleContainer =document.querySelector(`[data-key="${id}"]`);
//         // console.error("删除的moduleContainer",moduleContainer)
//         if(moduleContainer){
...
// /**
//  * 用于发布后暴露对外接口
//  * 由于vite的机制，会默认按如下规则处理摇树和别名
//     （一） 摇树：
//           1.如果模块没有被import过，整体剔除
//           2.对被import过的模块进行方法摇树，剔除代码中没有被本模块或者其他模块调用过的方法，无视是否export出去
//           3.被调用过的export方法，进入别名压缩机制。
//     （二）export方法别名化
//           1. 如果被作为参数调用过，则保持原名
//  *        2. 否则进行别名化，改成a、b、c之类的
//           3. 如果一个export方法，并未被其他模块导入，又被内部代码调用过，则export会被取消，转成成普通方法

//     该机制：
//         1. 导致了以普通模式暴露的api会失效。
//         2. 但可以通过console.log打印该方法来曲线救国,只不过不优雅。
//         3. 通过赋值不行，因为变量不使用，同样也会被摇树
//  * 
//  */
// // let ignore={showTip,showPage,showSlot,showPropertyBar,showToast}
// // console.debug("傲雪框架API:",ignore)