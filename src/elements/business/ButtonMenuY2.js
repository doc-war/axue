...
export class ButtonMenuY2 extends LitElement { 
    static properties = {
        args:{type: Object} 
    } 
    static styles = [
    ...
    render() {
        return html`
            <button class="button cursor" @click=${this._openMenu}  
                style="background-color:${this.args?.button?.backgroundColor??'white'};color:${this.args?.button?.color??'black'};">
                ${this.args?.button?.text??"菜单"}
            </button>
        `;
    }
    _openMenu(){
...
*/
AxueElement.preDefine( defaultTagName.ButtonMenuY2, ButtonMenuY2);
